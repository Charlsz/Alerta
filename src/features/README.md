...
Salida final: tabla `features_municipio_cultivo` con 27 variables (15 SPC + 6 SEP + 6 SVE). Además produce tablas auxiliares `features_ndvi`, `features_deforestacion`, `features_dane` y `features_clima` (15 vars SPC, incl. viento).
...
| `ndvi.py` | 54 | NDVI medio mensual + anomalía desde MODIS (HDX) |
 | `normalize.py` | 77 | Normalización min-max robusta (p1–p99) para las 27 variables |
...
| `spatial.py` | 114 | Join espacial estaciones IDEAM → municipios; crea `estaciones_municipio` y `municipios_geom` |
 | `store.py` | 99 | Une clima + producción + vulnerabilidad en la tabla maestra (27 variables) |
